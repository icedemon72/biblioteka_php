<?php
// upravljanje.php -> stranica gde imamo listu svih knjiga, kao i opciju da iste uredimo

// pokrecemo sesiju
session_start();

// ukoliko admin nije ulogovan
...
// pravimo asoc. niz $knjiga (vidi knjige.php)
$knjige = array(
  'id' => array(),
  'naziv' => array(),
  'autor' => array(),
...
// na pocetku, izmene nisu zavrsene, pa je $done = false
$done = false;

// ukoliko je izabrana neka knjiga, tj. ukoliko imamo informacije o knjizi u nasem URL-u
if (isset($_GET['knjiga'])) {
  // iz URL-a uzimamo ID knjige
  $knjigaId = $_GET['knjiga'];

  // IZABERI SVE IZ knjige GDE JE id = $knjigeId
  $sql = "SELECT * FROM knjige WHERE id = '$knjigaId'";
  $result = $conn->query($sql);

  // popunjavamo asocijativni niz, bitna napomena jeste da su kljucevi u asoc. nizu nizovi
  // ovde ih mi predefinisemo da budu string sto nije bas dobra praksa ali je jednostavnije
  // uraditi (mogao je da se napravi jos jedan asocijativni niz pa kasnije da se proverava
  //         da li je neka knjiga izabrana, sto dodatno komplikuje kod)
  ...
  // ukoliko je kliknuto dugme za izmenu knjige i ukoliko postoji $knjige['id'][0]
  // naravno, mi smo i dalje u if (isset($_GET['knjiga'])), sto znaci da ce ovaj if
  // moci da se pokrene SAMO AKO smo vec izabrali knjigu
  if (isset($_POST['submit']) && $knjige['id'][0]) {
    // informacije o knjizi, koje su nove
    $name = $_POST['name'];
    $author = $_POST['author'];
    $year = $_POST['year'];
    $number = $_POST['number'];
    // AZURIRAJ knjige POSTAVI DA JE (...) = (...) GDE JE id = $knjigaId
    ...
    $conn->query($sql);
    $done = true;
    // nakon uspesne izmene knjige, refresh-uj stranicu nakon 1s
    header('Refresh: 1, URL=./upravljanje.php');
  }

  // u suprotnom (ako nije izabrana jedna knjiga, tj. ako ne postoji GET metoda)
} else {
  // IZABERI SVE IZ knjige
  $sql = "SELECT * FROM knjige";

  // ovde proveravamo da li je uneseno nesto u Search bar, ukoliko jeste, menjamo
  // $sql promenljivu (tj. menjamo SQL upit na osnovu da li je uneseno nesto u polje za pretragu)
  if(isset($_GET['query'])) {
    // uzimamo vrednost pretrage
    $query = $_GET['query'];
    // IZABERI SVE IZ knjige GDE JE autor KAO %$query% ILI naizv KAO %$query%
    // vrsimo pretragu po autoru i nazivu knjige
    $sql = "SELECT * FROM knjige WHERE autor LIKE '%$query%' OR naziv LIKE '%$query%'";
  }

  // na kraju, ubacujemo sve u asoc. niz i prikazujemo podatke u HTML-u (vidi linije 160+)
  $result = $conn->query($sql);
  while ($row = $result->fetch_assoc()) {
...
<body class="login_body">
  ...
  <div class="container d-flex justify-content-center">
    <!-- Proveravamo da li "NIJE" izabrana samo jedna knjiga -->
    <?php if (!isset($_GET['knjiga'])): ?>
      <section class="row w-100 d-flex justify-content-start">
        <!-- Ukoliko nije izabrana samo jedna, izlistacemo sve knjige u karticama (pomocu bootstrap-a) -->
        <!-- Same informacije o knjigama imamo u asocijativnim nizovima -->
        <!-- Ova for petlja ide od 0 -> velicine $knjige['id'], tj. od 0 do broja knjiga u bazi podataka -->
        <?php for ($i = 0; $i < sizeof($knjige['id']); $i++): ?>
          ...
          <div class="col-sm-12 col-md-6 col-lg-4 d-flex justify-content-center">
            <div class="card my-2">
              ...
              <!-- Ispisujemo informacije o knjigama -->
              <div class="card-body">
    ...
    <!-- Ukoliko je izabrana samo jedna knjiga, trebamo dati mogucnost izmene iste -->
    <?php else: ?>
